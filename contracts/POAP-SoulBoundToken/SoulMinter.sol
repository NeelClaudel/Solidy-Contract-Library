...
contract SOULMinter is Ownable{
    /* ============ Events ============ */
    event SOULMinted(address indexed account, uint256 indexed eventId);
    ...
    /* ============ Constructor ============ */
    /*
     * @dev initialize SOUL addresss and signer address
     */
    constructor(address soulAddr_, address signer_){
        soul = SOUL(soulAddr_);
        signer = signer_;
    }
    ...
    external
    {
        ...
        soul.mint(account, eventId); // mint
        mintedAddress[eventId][account] = true; // 记录mint过的地址
        emit SOULMinted(account, eventId);
    }
