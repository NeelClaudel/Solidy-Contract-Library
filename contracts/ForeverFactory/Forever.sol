...
import "./ForeverFactory.sol";
import "hardhat/console.sol";

contract Forever {
    address public ForeverFactory;
    ...
    function placeTrade(uint _value, bool _long) external {
        ...
        safuFund += splitFee;
        if (splitFee > 0) IERC20(pUSD).transfer(ForeverFactory, splitFee);
    ...
    function liquidatePosition(address _user) external {
        ...
        safuFund += splitFee;
        if (splitFee > 0) {
            IERC20(pUSD).transfer(ForeverFactory, splitFee);
            IERC20(pUSD).transfer(msg.sender, splitFee);
            ForeverFactory(ForeverFactory).liquidated(_user, splitFee);
        }
    }
